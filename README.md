...
- **Завершение привычки** — кнопка 🏁 «Завершить» отмечает, что цель достигнута (отдельный статус от архива, с `completed_at`). Завершённые привычки уходят с дашборда и попадают во вкладку «Завершённые привычки» в Истории со своей итоговой статистикой.
- **Поиск и фильтрация** — поле поиска по названию и выпадающий список по типу привычки над списком привычек; backend поддерживает `GET /api/habits?search=&type=&view=active|archived|completed|all`.
- **История** — отдельная страница с двумя вкладками: «Отметки» (лента ежедневных выполнений по активным привычкам) и «Завершённые привычки» (список финишированных привычек с периодом, лучшим стриком и % выполнения).
- **Подробная аналитика** — рейтинг привычек по % выполнения, разбивка по дням недели, сравнение по месяцам, инсайты (лучший день недели, тренд месяц-к-месяцу).
- **Регистрация с защитой**:
  - почта принимается только с доменов `gmail.com`, `mail.ru`, `yandex.ru` (проверка и на фронте, и на backend через zod);
  - обязательное повторение пароля с серверной и клиентской проверкой совпадения;
  - обязательная галочка согласия с условиями использования (без неё кнопка регистрации не сработает);
  - после успешной регистрации — редирект на страницу логина (без автоматического входа), пользователь логинится сам.
